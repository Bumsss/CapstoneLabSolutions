...
export class ConsumableReportsComponent {
  ...
  constructor(private http: HttpClient,private ConsumableReportService: ConsumableReportService, private datePipe: DatePipe, private cdr: ChangeDetectorRef) { 
  ...
  fetchTransactions() {
    this.http.get("http://localhost:8085/api/consumableTrans/")
    .subscribe((resultData: any) => {
      ...
      this.TransactionArray.forEach((transaction: any) => {
        ...
        transaction.ConsumableName = this.getConsumableName(transaction.ConsumableID);
  ...
  register(){
    ...
    this.http.post("http://localhost:8085/api/consumableTrans/add", bodyData)
    .subscribe((resultData: any) => {
  ...
  UpdateRecords(){
    ...
    this.http.put("http://localhost:8085/api/consumableTrans/update" + "/" + this.currentID, bodyData)
    .subscribe((resultData: any) =>{
      console.log(resultData);
  ...
  setDelete (data: any){
    this.http.delete("http://localhost:8085/api/consumableTrans/delete" + "/" + data.TransactionEquipID)
    .subscribe((resultData:any) =>{
      console.log(resultData);
...
onCourseChange(): void {
  if (this.CourseID) { // Check if a course is selected
    this.ConsumableReportService.getConsumablesByCourseId(this.CourseID).subscribe(
      (response: any) => {
  ...
  loadConsumables(): void {
    ...
    getConsumableName(ConsumableID: number): string {
      const equipment = this.ConsumableArray.find((e) => e.ConsumableID === ConsumableID);
