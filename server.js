...
server.delete("/api/consumables/delete/:id", (req, res) => {
  let sql =
    "DELETE FROM tblConsumable where ConsumableID=" + req.params.id + "";
  let query = db.query(sql, (error) => {
    if (error) {
      res.send({ status: false, message: "Record Delete Failed!" });
    } else {
...
server.get("/api/users/search/:studentNum", (req, res) => {
  ...
  db.query(sql, function (error, result) {
    if (error) {
      ...
      res
        .status(500)
        .send({ status: false, message: "Error searching student number" });
    } else {
      if (result.length > 0) {
...
server.delete("/api/equipmentTrans/delete/:id", (req, res) => {
  let sql =
    "DELETE FROM tblTransactionEquipment where TransactionEquipID=" +
    req.params.id +
    "";
  let query = db.query(sql, (error) => {
    if (error) {
...
server.delete("/api/access/delete/:id", (req, res) => {
  let sql =
    "DELETE FROM tblAccessLevel where AccessLevelID=" + req.params.id + "";
  let query = db.query(sql, (error) => {
    if (error) {
